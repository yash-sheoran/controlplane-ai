...
# Section 10.2's "Safety Violation Rate"/"Bias Detection Rate" both say
# "configured threshold" without stating a number. Section 2C's own risk
# bands put 7-8 as "High" risk, and Section 3 Step 8's example rules use 7
# as a human_review-level trigger for safety_risk — 7 is used here as the
# default "violation" threshold for both, consistent with that banding.
DEFAULT_VIOLATION_THRESHOLD = 7
...
def decision_distribution(since=None):
...
def _response_metrics_values():
    return [
        r["response_metrics"]
        for r in AuditRecord.objects.values("response_metrics")
...
def latency_percentiles():
    ...
    across all requests." """
    latencies = sorted(
        rm["latency_ms"] for rm in _response_metrics_values() if "latency_ms" in rm
    )
    return {
...
def total_cost_today():
    ...
    records = AuditRecord.objects.filter(timestamp__gte=today_start).values("response_metrics")
    return round(
...
def cost_per_model():
    ...
    budget and why." """
    totals = {}
    for rm in _response_metrics_values():
...
def active_human_review_queue_count():
    """Section 10.1: "Active Human Review Queue: Current number of
    pending human review cases." """
    return AuditRecord.objects.filter(human_review_status="PENDING").count()
...
def hallucination_rate(days=7):
    ...
    scores = [
        ...
        for r in AuditRecord.objects.filter(timestamp__gte=since).values("audit_quality")
        if r["audit_quality"] and "hallucination_risk_score" in r["audit_quality"]
    ]
    return round(sum(scores) / len(scores), 4) if scores else None


def safety_violation_rate(threshold=DEFAULT_VIOLATION_THRESHOLD, days=7):
    """Section 10.2: "Safety Violation Rate: Percentage of requests
    triggering safety_risk >= configured threshold." """
    since = timezone.now() - datetime.timedelta(days=days)
    total = 0
    violations = 0
    for r in AuditRecord.objects.filter(timestamp__gte=since).values("audit_responsibility"):
...
def data_leakage_attempts(threshold=DEFAULT_VIOLATION_THRESHOLD, days=7):
    """Section 10.2: "Data Leakage Attempts: Count of responses where
    data_leakage_risk_score >= 7, by use-case." (segmentation by use-case
    is available via the use_case_id parameter below)."""
    since = timezone.now() - datetime.timedelta(days=days)
    count = 0
    for r in AuditRecord.objects.filter(timestamp__gte=since).values("audit_responsibility"):
        ar = r["audit_responsibility"]
        if ar and ar.get("data_leakage_risk_score", 0) >= threshold:
            count += 1
    return count


def bias_detection_rate(threshold=DEFAULT_VIOLATION_THRESHOLD, days=7):
...
def blocked_request_rate(days=7):
...
def retry_verify_rate(days=7):
    """Section 10.2: "Retry/Verify Rate: Percentage of requests requiring
    at least one retry, indicating model reliability." Uses the objective
    retry_count metric (Section 3 Step 5), not the VERIFY policy action,
    since the doc specifically says "requiring at least one retry"."""
    since = timezone.now() - datetime.timedelta(days=days)
    total = 0
    retried = 0
    for r in AuditRecord.objects.filter(timestamp__gte=since).values("response_metrics"):
...
def false_positive_rate_by_dimension(dimension, use_case_id=None, days=7):
    ...
    if use_case_id:
        qs = qs.filter(trace__use_case__use_case_id=use_case_id)
...
def simulate_threshold_change(use_case_id, bucket, dimension, new_threshold, days=7):
    ...
    qs = AuditRecord.objects.filter(
        timestamp__gte=since, trace__use_case__use_case_id=use_case_id,
    )
